...
class ILI9341:
    ...
    def draw_sprite(self, buf, x, y, w, h):
        ...
        # Iterate through the sprite buffer and draw each pixel
        for i in range(h):
            for j in range(w):
                # Calculate the pixel position in the buffer
                index = (i * w + j) * 2  # Each pixel is 2 bytes (RGB565)
                pixel_data = buf[index : index + 2]  # Get the pixel data (2 bytes)

                # Extract the RGB565 color values
                color = ((pixel_data[0] << 8) | pixel_data[1]) & 0xFFFF
